...
def find_min_box(obj_list_,names,color_frame_,depth_frame_):
    global plane_x,plane_y,plane_z
    box_min = 100000000
    obj_min = obj_list_[0]
    #find min obj
    for i in range(len(obj_list_)):
        if(names[i]=='0'):
            box_area = abs((obj_list_[i]['xmax'] - obj_list_[i]['xmin'])*(obj_list_[i]['ymax'] - obj_list_[i]['ymin']))
            if box_area < box_min:
                box_min = box_area
                obj_min = obj_list_[i]
    ...
    # delta_rate?
    # length and width min & max compressed
    delta_rate=0.3
    ...
    try:
        while True:
            ...
            if is_async_mode:
                ...
                pic_time = time()
                print('###########pic_time is {} ####################'.format(pic_time))
            ...
            for obj in objects:
                ...
                #添加obj
                obj_list.append(obj)
                class_name.append(det_label)

            # the number of classes
            print(class_name)

            if len(obj_list) > 0:
                objmin = find_min_box(obj_list,class_name,frame,depth_frame)
                cv2.putText(frame,
            ...
            end_time = time()
            print('###########end_time is {} ####################'.format(end_time-pic_time))
            if len(data)==5:
                print("now send msg ......")
                send_data = [int(plane_x*1000), int(plane_y*1000), int(plane_z*1000), str(pic_time), str(end_time)]
                print(send_data)
